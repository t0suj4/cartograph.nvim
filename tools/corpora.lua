...
return {
    server = {
        ...
        rev = '4743238408a4',
        lang = 'java',
        expected = { refs = 80414, nodes = 109386 }, -- measured 2026-09-24 at v195 (CART-0675): graphs IDENTICAL per item against the v191 snapshot, which stays — this root IS a source root (server/src/main/java), so the module-segment fix cannot reach it. Prior: recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs -377, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-09-05 (v169 AN ANNOTATION IS NOT EVIDENCE IN A NAME CONTEST, CART-0703; attributed by BISECT on libs, CART-0757): -115 refs, NODES UNCHANGED — THE SAME COMMIT AND THE SAME CAUSE AS libs' -9, and the arithmetic closes: e45abf8's own message measured `server edges 223427 -> 223312`, which IS -115. ★ THE LOSS IS THE FIX: `@SuppressForbidden`, a code-review marker that registers nothing, was silently acting as java's overload discriminator — being `cbarg` withdrew its holder from same-file priority and left the other overload a unique winner. Hand-adjudicated on libs' JarHell.java: right 3 times, WRONG TWICE, arity never consulted. Recovering them properly is CART-0676 (arity). ⚠ BOTH PINNED JAVA CORPORA WERE LEFT RED BY THAT COMMIT and neither was recalibrated, so the pins have been failing on an INTENDED change for two days — and a standing red masks every subsequent drift, which is the actual harm. Δnodes 0 is the tell: refs fell while the population held, so this is LOST RESOLUTION, not a population change. Prior: 80906 -- recalib 2026-09-03 (v165 A PARAMETERIZED SUPERTYPE IS STILL A SUPERTYPE, CART-0672; re-save CART-0716): +637 refs, NODES UNCHANGED. `extends Base` parses `superclass -> type_identifier` and `extends Base<T>` parses `superclass -> generic_type -> type_identifier`; java's super_query named only the first, so a GENERIC class had no parent and every receiver typed by one lost the chain. 672 edges added, 825 calls changed, and in the printed sample EVERY transition is `refused (ambiguous) => to <Supertype>::<member>` -- the gain comes OUT OF THE REFUSED BUCKET, which is the sound-first direction. ★ NODES UNCHANGED IS THE TELL: v165 recovers RESOLUTION, not population, so refs rise while the node count holds. A node delta here would have meant something else did this. ★ ATTRIBUTED BY MEASUREMENT, NOT BY DATE, and this is why the re-save waited: the snapshot was written at extraction VERSION 159, so it pooled EIGHT versions and CART-0672's own commit message refused to bless it -- "the blast radius beyond libs is unmeasured and blessing it would be the 're-baseline whatever drifted unread' failure". Resolved in one run: 9300970 (the commit before 77df2b4, cache v164) diffs CLEAN against the v159 snapshots on libs, server AND synjava -- MATRIX: PASS -- so v160-v164 moved java by zero, turning four provenance CLAIMS into measurements. v166 (CART-0701) and v167 (CART-0674) were each measured at zero on this corpus in both directions. That leaves v165 as the sole cause of 100% of the drift. ★ HAND-READ WITNESS, the one the report asked to be counted: `HierarchicalKMeans.java -> KMeansResult::centroids` went 4 -> 13 occurrences; the file holds 21 `centroids()` call sites over five receivers, and `class KMeansIntermediate<V> extends KMeansResult<V>` is the parameterized superclass that was invisible. Prior: 80269 -- recalib 2026-08-23 (v154 A JAVA FIELD WITHOUT AN INITIALIZER IS STILL A FIELD, CART-0537): +15940 nodes, refs unchanged. The `vars` query required `value: (_)`, so every field declared without an initializer was INVISIBLE as a var — 1130 of 2656 declarators on libs, 42.5%. They are now minted from a second pattern using the negated-field assertion `!value`, and carry `decl`, the field a C prototype already uses for "declared, not defined". Δrefs 0 is the tell that these are new NODES and not re-pointed edges. ★ THE POPULATION IS NOT RANDOM: `private final byte[] idPage;` is assigned in a CONSTRUCTOR, so the invisible 42.5% was enriched in exactly the cases the write axis has most to say about — libs' ladder moves single-writer 83 -> 987 and multi-writer 45 -> 221 while const barely moves (1221 -> 1243). Every java var-level number before this was over ~57% of the field population. Prior: 93446 -- recalib 2026-08-22 (v132 A JAVA LAMBDA IS A FUNCTION AND NOW HAS A NODE, CART-0406 — GATED HERE LATE, CART-0502): +6545 nodes / +1640 refs. THIS PIN WAS RED ON CLEAN MAIN, which is the whole reason CART-0502 exists: v132 landed on 08-14 recalibrated on `libs` and NOT here, so for eight days every server diff was a mixture that read as attributable. ATTRIBUTED BY MEASUREMENT, not by date: 6359 of the +6545 nodes (97.1%) carry `#cb` in their id — the `<callee>#cb` name v132 mints for an anonymous lambda in argument position — and `libs`, the corpus that DID get the v132 recalib, gives 1538 of its recorded +1579 (97.4%). The same ratio on a corpus whose delta is known is what makes this an attribution and not a coincidence; the ~3% residual is the lambda-assigned-to-a-named-variable share, which takes the declarator's name instead. ★ v134 (.h PARSED AS C, the era libs' own comment names) IS RULED OUT, not merely unlikely: `server/src/main/java` holds 4843 .java files and ZERO non-java files, so a C++-header fix cannot reach it. libs is java WITH a native tail (28 .cc, 6 .h) and server is the same repo without one — the property does not travel between subtrees, and reading libs' comment as if it did is how v134 got suspected in the first place. Prior: 78629/86901 -- recalib 2026-08-07 (v119 FIVE LANGUAGES HAD NO ENCLOSING FUNCTION, CART-0306): -340 nodes / -1496 edges / +137 edges, Δrefs 0. `in_function` read `spec.fn_types or DEFAULT` and the default names function_definition/function_declaration — neither of which exists in java, so it returned nil for EVERY node and could not reject a declaration made inside a function. What went: the same java anonymous-class fields as libs, at scale. The +137 edges are re-pointing, not growth — refs is unchanged at 78629, so references that had matched a fabricated module variable now match a real target. Δrefs 0 is the tell: these nodes were fabricated MODULE variables that nothing correctly referenced, so removing them moved no reference. Only java and rust moved, and the predicate is sharp — a language changes iff its `vars` query is UNROOTED AND its in_function was blind; everyone else roots at source_file/program and never needed the predicate. Prior: nodes 87241 -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +1502 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. java's `region@0` owns STATIC FIELD INITIALIZERS — the load-time code of a class (TransportVersion.java's constants calling loadConstant/collectFromResources). Prior: 77127 -- +18 @ honesty pass v46 (stale baseline caught up on prior Java refused→resolved)
    ...
    libs = {
        ...
        rev = '4743238408a4',
        lang = 'java',
        expected = { refs = 11613, nodes = 16512 }, -- re-save 2026-09-24 (v195 A JAVA IMPORT RESOLVES PAST A MAVEN MODULE SEGMENT, CART-0675): refs +0, nodes +0; the SNAPSHOT moved by 1,279 edges ADDED, 0 removed, all `import`, equal to the java import-edge count through the provider (0 before: libs is an aggregator, and every cross-module import was lost). Witness: cli/src/main/java/org/elasticsearch/cli/Command.java imports cli.terminal.Terminal, core.SuppressForbidden, logging.Level, and all three edges are among the added. Refs unchanged is the tell that import edges do not feed java ref resolution today. Prior: recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs -31, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-09-05 (v169 AN ANNOTATION IS NOT EVIDENCE IN A NAME CONTEST, CART-0703; attributed by BISECT, CART-0757): -9 refs, NODES UNCHANGED. ★ THE LOSS IS THE FIX, and e45abf8 hand-read every one of the nine before landing: all are same-file calls to OVERLOADED methods, and THE OVERLOAD WAS BEING PICKED BY WHICH ONE LACKS A @SuppressForbidden — a code-review marker that registers nothing, silently acting as java's overload discriminator because being `cbarg` withdrew its holder from same-file priority and left the other overload a unique winner. Adjudicated in JarHell.java: right 3 times, WRONG TWICE (`checkJarHell(parseClassPath(), output)` -> the 1-arg overload; `parseClassPath(System.getProperty(..))` -> the 0-arg one), arity never consulted. Giving up nine name-matched resolutions to stop a silent wrong-answer generator is the sound-first direction; recovering them properly is CART-0676 (arity). ⚠ WHY THIS SAT RED FOR TWO DAYS: the commit MEASURED the -9 and did not recalibrate, so the pin has been failing on an INTENDED change ever since — and a standing red masks every subsequent drift, which is the actual harm. Attribution here was a 5-run bisect over the 15 commits since 87da5cc: 9e77c9b GREEN at 11653, e45abf8 RED at 11644. Δnodes 0 is the tell — refs fell while the population held, so this is LOST RESOLUTION, not a population change. Prior: 11653 -- recalib 2026-09-03 (v165 A PARAMETERIZED SUPERTYPE IS STILL A SUPERTYPE, CART-0672; re-save CART-0716): +4 refs, NODES UNCHANGED. All four were read in the source when v165 landed and are restated here because a number without its witness is not a calibration: `MultiPoint extends GeometryCollection<Point>` (hasZ inherited, eight classes in the package define it, so the name alone refused as ambiguous) and `ConstructingObjectParser extends AbstractObjectParser` (parseArray inherited). Every one is `refused (ambiguous) => resolved`; 0 lost, 0 redirected. ★ The same v164 control run that attributed `server` covers this corpus: 9300970 diffs CLEAN here too, so nothing between v160 and v164 touched it. Prior: 11649 -- recalib 2026-08-23 (v154 A JAVA FIELD WITHOUT AN INITIALIZER IS STILL A FIELD, CART-0537): +1093 nodes, refs unchanged. The `vars` query required `value: (_)`, so every field declared without an initializer was INVISIBLE as a var — 1130 of 2656 declarators on libs, 42.5%. They are now minted from a second pattern using the negated-field assertion `!value`, and carry `decl`, the field a C prototype already uses for "declared, not defined". Δrefs 0 is the tell that these are new NODES and not re-pointed edges. ★ THE POPULATION IS NOT RANDOM: `private final byte[] idPage;` is assigned in a CONSTRUCTOR, so the invisible 42.5% was enriched in exactly the cases the write axis has most to say about — libs' ladder moves single-writer 83 -> 987 and multi-writer 45 -> 221 while const barely moves (1221 -> 1243). Every java var-level number before this was over ~57% of the field population. Prior: 15419 -- recalib 2026-08-14 (v134 .h PARSED AS C, CART-0410): +267 refs / -22 nodes. libs is java with a small native tail (28 .cc, 6 .h); the tree holds C++ source so its headers now parse as C++ too. Small because the corpus is mostly java — which is the point of keeping a mixed corpus pinned: a change aimed at C++ should move it a little and not at all elsewhere. Prior: 11382/15441 -- recalib @ CART-0406: +1579 nodes / +394 refs = A JAVA LAMBDA IS A FUNCTION AND NOW HAS A NODE. flow stops at `lambda_expression` on the documented promise that a node is MINTED to hold the rows; an anonymous CLASS kept it (its `run` was always in the graph) and a lambda did not, so 1670 lambda bodies here had NO ROWS IN NO NODE. The +1579 are named js's way — the declarator's name, or `<callee>#cb` in argument position (map#cb, filter#cb, forEach#cb) — because js solved this for arrows and a second convention would BE the drift. +394 refs = calls inside those bodies finally have an OWNER. The gap between 1670 lambdas and 1579 nodes is deliberate: a lambda in RETURN position has no name and no enclosing call, and `fn#cb` in a name-keyed graph carries no information. EVERY number rose and none fell, which is the review question answered by the shape of the delta. Found by the combinatorial grid (CART-0405) the day an `inlambda` SHELL existed. Prior: refs 10988, nodes 13862 -- recalib 2026-08-07 (v119 FIVE LANGUAGES HAD NO ENCLOSING FUNCTION, CART-0306): -24 nodes / -65 edges, Δrefs 0. `in_function` read `spec.fn_types or DEFAULT` and the default names function_definition/function_declaration — neither of which exists in java, so it returned nil for EVERY node and could not reject a declaration made inside a function. What went: fields of ANONYMOUS CLASSES declared inside methods — ReleasableIterator.java's `private T value` lives in a `new ReleasableIterator<>() { … }` inside a static method, and was published as a file-level variable named `value`. Δrefs 0 is the tell: these nodes were fabricated MODULE variables that nothing correctly referenced, so removing them moved no reference. Only java and rust moved, and the predicate is sharp — a language changes iff its `vars` query is UNROOTED AND its in_function was blind; everyone else roots at source_file/program and never needed the predicate. Prior: nodes 13886 -- recalib 2026-08-06 (v117 A COMMENT IS NOT A STATEMENT IN JAVA/RUST, CART-0304): -2 nodes. flow + the extractor skipped only the node name `comment`, which java and rust do not use (`line_comment`/`block_comment`, + rust `doc_comment`), so every comment was a STATEMENT. The node delta is almost entirely a RE-KEY, not a loss: a file's module-level `region@0` becomes `region@<first real line>` because the license header no longer opens the statement run (ArrowUtils.java region@0 -> region@9). The net -2 are two files whose only statement-run content WAS comments, so they now correctly have no module-level region at all. Δrefs 0 in both — a comment carries no identifier, which is exactly why this hid: it moved statement PARTITION and nothing else. Found by tools/langaudit.lua, not by a test; the suite is lua-only and lua calls its comments `comment`. Prior: nodes 13888 -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +166 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. In java the region is `region@0` and what it owns is STATIC FIELD INITIALIZERS — java's load-time code, structurally the same thing as a lua module body (Terminal.java's DEFAULT = defaultTerminal()). Prior: 10822 -- +1 @ generic Class<T> return (getLibrary(VectorLibrary.class))
    ...
    sylius = {
        ...
        rev = '9b6799e2b884',
        expected = { refs = 5782, nodes = 32237 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs -10, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-09-07 (v189 THE UNIQUE-OWNER RUNG, CART-0806): +35 refs, +10 nodes. A member name that belongs to EXACTLY ONE callable type in the distilled surface, and that this project NEVER NAMES, types its own receiver: `elem.getAttribute()` is Element's. ★★ WHAT REFUSES IS THE POINT — `get` has 14 callable owners, `delete` 13, `has` 11, `add` 10, `set` 9, `addEventListener` 230, so the rung declines exactly the names a project is most likely to use for its own API. Resolutions carry `inferred` (the TYPE-INFERRED tier): the member is certain, the RECEIVER is a hedge. Purely additive, 0 removed; 48 sampled sites across jquery and converse.js were read and all 48 correct. Prior: recalib 2026-09-06 (v188 THE BROWSER SURFACE, CART-0805): +45 refs, +12 nodes. `tools/domdistill.lua` reads TypeScript's own lib.dom.d.ts into dom-api.mpack — 27 globals, 18873 keys, extends chains expanded so `document.appendChild` (declared seven interfaces up, on Node) is a key — and spec/profile/node.lua unions it, because select_env gives a root ONE profile and a repo is routinely both. ★★ AND THE GATE THAT HAD TO MOVE FIRST: the stdlib TAIL word list refused every one of these calls as `refused: vocab` before the surface holding the exact dotted key was asked. An oracle that confirms the whole path outranks a hand-written tail word. Purely additive, 0 removed. Prior: ⚠ a PHP corpus. recalib 2026-09-06 (v182 THE NODE/JS ENVIRONMENT PROFILE, CART-0800): +106 refs, +2 nodes. The `node-package` shape now activates `spec/profile/node.lua`, distilled from the installed engine by tools/nodedistill.lua (161 namespaces, 3959 members, 26 free globals). Unresolved `assert.equal` / `Math.floor` / `parseInt` calls disposition to `stdlib` and MINT against the oracle. sylius is php+js, same shape as grocy. ★ EVERY MINTED NODE WAS VERIFIED AGAINST `node` ITSELF: 86 of 86 on ghost, 0 bad. Prior: -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +1 ref (nodes unchanged) = a top-level call now owned by its statement-run REGION. The smallest lift in the grid, and it is the right answer: symfony php is class bodies with no file-scope statements, so the single edge is in a .js asset (statistics_chart.js::region@108 -> renderChart). Prior: 5605 -- recalib 2026-07-19: +53 nodes/+10 refs = JS/TS pivot (index.js webpack configs class-keyed SyliusShop._prepareWebpackConfig) + anon-fn #cb restructuring + 1 php fn-value honesty; php side stable
        lang = 'php',
    ...
    ghost = {
        ...
        rev = 'f7d7df8f9816',
        expected = { refs = 42510, nodes = 40999 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs -631, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-09-07 (v189 THE UNIQUE-OWNER RUNG, CART-0806): +2113 refs, +25 nodes. A member name that belongs to EXACTLY ONE callable type in the distilled surface, and that this project NEVER NAMES, types its own receiver: `elem.getAttribute()` is Element's. ★★ WHAT REFUSES IS THE POINT — `get` has 14 callable owners, `delete` 13, `has` 11, `add` 10, `set` 9, `addEventListener` 230, so the rung declines exactly the names a project is most likely to use for its own API. Resolutions carry `inferred` (the TYPE-INFERRED tier): the member is certain, the RECEIVER is a hedge. Purely additive, 0 removed; 48 sampled sites across jquery and converse.js were read and all 48 correct. Prior: ★ resolution 43.5 -> 45.5% — smaller than the browser corpora because a server's unresolved mass is npm, not the platform. recalib 2026-09-06 (v188 THE BROWSER SURFACE, CART-0805): +1237 refs, +24 nodes. `tools/domdistill.lua` reads TypeScript's own lib.dom.d.ts into dom-api.mpack — 27 globals, 18873 keys, extends chains expanded so `document.appendChild` (declared seven interfaces up, on Node) is a key — and spec/profile/node.lua unions it, because select_env gives a root ONE profile and a repo is routinely both. ★★ AND THE GATE THAT HAD TO MOVE FIRST: the stdlib TAIL word list refused every one of these calls as `refused: vocab` before the surface holding the exact dotted key was asked. An oracle that confirms the whole path outranks a hand-written tail word. Purely additive, 0 removed. Prior: ghost SERVES A BROWSER FRONTEND out of a node server (core/frontend/public), which is the measurement that decided the union: the DOM half pays on the server corpus too, and costs it nothing. recalib 2026-09-06 (v186 THE ORACLE ANSWERS FOR A PATH, CART-0804): +3 refs, +1 node. ⚠ THIS IS LATE: v186 shipped WITHOUT gating ghost, on the reasoning that the change is inert where no npm surface confirms a member — and it is, for the npm half. The node-api surface confirms members too, and `mint_path` asking the oracle for a whole dotted path reaches three of them here. A control run in a HEAD worktree attributed the drift to v186 exactly, not to the CART-0808 chain work that found it, which is byte-identical on this corpus without an artifact. ★ THE LESSON IS THE ONE THE CHANGE ITSELF WAS ABOUT: "inert without a surface" was true of the surface I was thinking of. Prior: recalib 2026-09-06 (v183 A BARE SPECIFIER BINDS A PACKAGE, CART-0800): +1 ref. ★ ONE EDGE, AND THE NUMBER IS THE POINT: the package-bind rewrite is gated on the active profile CONFIRMING the member, and without an npm surface installed only the node namespaces can confirm anything — so it fires once. With a surface it reaches the 61.5% of ghost's bare-specifier binds whose local name DIFFERS from the package (`_` <- lodash, `jwt` <- jsonwebtoken). The mechanism is verified, the SURFACE is the limiter. Prior: -- recalib 2026-09-06 (v182 THE NODE/JS ENVIRONMENT PROFILE, CART-0800): +8685 refs (+27.9%), +86 nodes. The `node-package` shape now activates `spec/profile/node.lua`, distilled from the installed engine by tools/nodedistill.lua (161 namespaces, 3959 members, 26 free globals). Unresolved `assert.equal` / `Math.floor` / `parseInt` calls disposition to `stdlib` and MINT against the oracle. ghost is the headline: `assert` ALONE was 13182 unresolved calls, 10% of the corpus's entire call population, from one import. Resolution 29.7% -> 42.3%. ★ EVERY MINTED NODE WAS VERIFIED AGAINST `node` ITSELF: 86 of 86 on ghost, 0 bad. Prior: -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +2647 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. js anon callbacks ALREADY had owners (`#cb` nodes, @adef), so these are true module bodies: bin/*.js CLI scripts and config files whose whole point is load-time work. Prior: 28320 -- recalib 2026-07-26: +14 refs (nodes unchanged) = v106 RECEIVER-PATH AGREEMENT — a candidate whose own qualified name is a segment-suffix of the call's wins when unique. +14 calls, EVERY ONE previously an ambiguous refusal; 0 lost, 0 redirected. e.g. `this.#MemberLinkClickEvent.create()` and `OfferAmount.OfferTrialAmount.create()`. Prior: 28306
        lang = 'javascript',
    ...
    ruby = {
        ...
        rev = 'ed0f92c5e779',
        expected = { refs = 1597, nodes = 2791 }, -- recalib 2026-09-06 (v182 THE NODE/JS ENVIRONMENT PROFILE, CART-0800): +3 refs, NODES UNCHANGED. The `node-package` shape now activates `spec/profile/node.lua`, distilled from the installed engine by tools/nodedistill.lua (161 namespaces, 3959 members, 26 free globals). Unresolved `assert.equal` / `Math.floor` / `parseInt` calls disposition to `stdlib` and MINT against the oracle. rails-the-repo's own JS. Nodes unchanged is the tell: this adds resolution, not population. ★ EVERY MINTED NODE WAS VERIFIED AGAINST `node` ITSELF: 86 of 86 on ghost, 0 bad. Prior: -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +4 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. Only 4 because activesupport/lib is framework SOURCE — class bodies, not statement runs (ForkTracker.hook! at file scope is the shape). Prior: 1590 -- recalibrated @ R5b-ivar (v78)
        lang = 'ruby',
    ...
    rails = {
        ...
        rev = '28b003a38d82',
        expected = { refs = 7508, nodes = 5201 }, -- recalib 2026-09-06 (v182 THE NODE/JS ENVIRONMENT PROFILE, CART-0800): +9 refs, NODES UNCHANGED. The `node-package` shape now activates `spec/profile/node.lua`, distilled from the installed engine by tools/nodedistill.lua (161 namespaces, 3959 members, 26 free globals). Unresolved `assert.equal` / `Math.floor` / `parseInt` calls disposition to `stdlib` and MINT against the oracle. discourse carries BOTH `config/application.rb` and a package.json; the rails profile still wins (nearest dir, and node-package is LAST), so only its JS files move. ⚠ A FIRST CUT PUT THE SHAPE ABOVE rails AND COST THIS GATE 4848 REFS — the ordering is the fence. ★ EVERY MINTED NODE WAS VERIFIED AGAINST `node` ITSELF: 86 of 86 on ghost, 0 bad. Prior: -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +20 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. All 20 are `require` at the top of an app/models/*.rb — modest here because a rails model file is a class body, not a statement run (contrast rspec, +2122). Prior: 7479 -- recalib @ UP-direction profile activation (v99): +4837 refs/+285 nodes = the ruby-rails profile now activates via ancestor-probing (config/application.rb 2 levels up in discourse/) → framework surface MINTS (ActiveRecord/ActiveSupport/ActionView), 0 edges removed, 0 project defs stolen (find yields to 5 project defs). Prior 2642 @ R5b-more (v98, pack-only, profile not activated on the subdir root)
        lang = 'ruby',
    ...
    go = {
        ...
        rev = '5a5f4a549522',
        expected = { refs = 9916, nodes = 11769 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs +4, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-08-29 (v159 IMPLICIT IMPORT BIND, CART-0624): +120 refs, nodes unchanged. `spec.import_bind_path` gives go's bare `import "net/http"` the local name it binds (`http`), 639 of 646 import edges now carry one, and a bind feeds REQUALIFICATION — so a dotted receiver whose package is bound resolves where it previously could not. Δrefs == edges ADDED, 0 removed, which is the shape to expect from a pin: it decides undecided edges, it does not re-decide decided ones. ⚠ NOT a measure of correctness — fabcensus on this corpus reports agreement 99.34% over the 304 edges the binding now DECIDES (302 pinned / 2 contradicted), and that number is the one to watch, not this total. Prior: 9754 -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +96 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. go's share is package-level `var x = f()` initializers (config/allconfig/alldecoders.go::region@13 dominates) + check.sh. Prior: 9658 -- recalib 2026-07-26: +65 refs (nodes unchanged) = v106 RECEIVER-PATH AGREEMENT — a candidate whose own qualified name is a segment-suffix of the call's wins when unique. +69 calls, EVERY ONE previously an ambiguous refusal; 0 lost, 0 redirected. e.g. `h.PathSpec.RelURL()` -> PathSpec.RelURL (go's embedded-field idiom names the type in the receiver path). Prior: 9593
        lang = 'go',
    ...
    odin = {
        ...
        rev = '967c6046a624',
        expected = { refs = 47555, nodes = 45271 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs +0, nodes +54. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-08-30 (v164 ODIN ATTRIBUTES HID A FIFTH OF THE STANDARD LIBRARY, CART-0630): +8994 nodes (+24.8%), +26785 refs (+129%). Odin attaches `@(require_results)` / `@(private="file")` to a declaration and the grammar makes `attributes` the FIRST NAMED CHILD, so the `.` anchor in the functions query failed and the proc matched NOTHING — not a def, not a node, no fn_range. MEASURED on core: 40842 procedure_declaration, 8887 beginning with `attributes` = 21.8% INVISIBLE. This is not a re-key like the two recalibrations below: those moved node IDENTITY at constant population, this ADDS a fifth of the corpus's functions and every edge that hangs off them. Hand-checked one witness before saving — `index :: proc` at core/bytes/bytes.odin:610 is real, same file and package, and calls to it were previously UNRESOLVED. Also dissolved CART-0629's fabricated `os/temp_file.odin -> os/path.odin::dir@394`, because the parameter's enclosing proc is now a def. Prior: refs 20770 nodes 36223 -- recalib 2026-08-06 (v117 A COMMENT IS NOT A STATEMENT, CART-0304): -898 nodes, Δrefs 0. ODIN IS THE THIRD LANGUAGE THE COMMENT FIX MOVED, and I missed it in the first control sweep by misreading the audit: it reported `comment` as a node type in odin, which is TRUE and INCOMPLETE — odin has BOTH `comment` and `block_comment` (as does scheme), so its `/* */` blocks were still statements after the java/rust fix landed. The lesson is the same shape as the bug: "grammar X has this node" does not mean "grammar X has ONLY this node". Same re-key signature as libs/rust — a module-level `region@0` becomes `region@<first real line>` — and Δrefs 0, because a comment carries no identifier. Prior: nodes 37121 -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +1244 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. +6%: like zig, odin puts procedure-valued declarations at package/container scope (bufio/reader.odin::region@337 wiring reader_read/reader_destroy into an io vtable). Prior: 19526 -- recalibrated @ odin node-local tearing (v88)
        lang = 'odin',
    ...
    python = {
        ...
        rev = 'c0608e0d167e',
        expected = { refs = 1854, nodes = 3891 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs -2, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-09-07 (v189 THE UNIQUE-OWNER RUNG, CART-0806): +25 refs, +5 nodes. A member name that belongs to EXACTLY ONE callable type in the distilled surface, and that this project NEVER NAMES, types its own receiver: `elem.getAttribute()` is Element's. ★★ WHAT REFUSES IS THE POINT — `get` has 14 callable owners, `delete` 13, `has` 11, `add` 10, `set` 9, `addEventListener` 230, so the rung declines exactly the names a project is most likely to use for its own API. Resolutions carry `inferred` (the TYPE-INFERRED tier): the member is certain, the RECEIVER is a hedge. Purely additive, 0 removed; 48 sampled sites across jquery and converse.js were read and all 48 correct. Prior: recalib 2026-09-06 (v188 THE BROWSER SURFACE, CART-0805): +1 refs, +1 nodes. `tools/domdistill.lua` reads TypeScript's own lib.dom.d.ts into dom-api.mpack — 27 globals, 18873 keys, extends chains expanded so `document.appendChild` (declared seven interfaces up, on Node) is a key — and spec/profile/node.lua unions it, because select_env gives a root ONE profile and a repo is routinely both. ★★ AND THE GATE THAT HAD TO MOVE FIRST: the stdlib TAIL word list refused every one of these calls as `refused: vocab` before the surface holding the exact dotted key was asked. An oracle that confirms the whole path outranks a hand-written tail word. Purely additive, 0 removed. Prior: recalib 2026-09-06 (v185 A SHAPE'S TWO CONFIG SOURCES COMPOSE, CART-0802): +3 refs, +2 nodes. `shapes.probe` joined a shape's static `config` and its `configure(root)` with an `or`, so node-package's `profile = 'node'` was thrown away on every package.json naming a `main` — and this corpus's does. The v182 node profile had therefore only ever activated on ghost, whose manifest names no entry. ★ THE TELL WAS A NUMBER THAT DID NOT MOVE: jquery and converse.js reported their pre-profile resolution to the decimal and I had a plausible wrong story ready for it. Purely additive, 0 removed; every minted name (parseInt, setTimeout) verified callable against `node` itself, and each is ECMAScript or a timer, so the claim holds for browser code too. ⚠ django-oscar is a PYTHON corpus and it moved: a JS front-end change is never a JS-corpora-only change (the v104 lesson). Prior: -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +244 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. +15%, the largest share of any non-ruby corpus: django's whole loading idiom is module-level (`get_model`/`get_class`/`is_model_registered` at the top of every models.py/admin.py). Prior: 1583 -- recalib 2026-07-26: v104 js member-target function literals (+12 refs/+11 nodes defs): `X.y = function(){}` now mints, so pre-class exports are definitions. MEASURED per-item vs a HEAD baseline: recovered 10, +3 out of a refusal, 0 redirected, 0 LOST — every added node is a .js file (a mixed repo, not a python change). Prior: refs 1571 / nodes 3872 -- +enclosing-chain captured-callable wins @ v51
        lang = 'python',
    ...
    jquery = {
        ...
        rev = 'b043db95042b',
        expected = { refs = 1297, nodes = 880 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs -22, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-09-07 (v189 THE UNIQUE-OWNER RUNG, CART-0806): +157 refs, +13 nodes. A member name that belongs to EXACTLY ONE callable type in the distilled surface, and that this project NEVER NAMES, types its own receiver: `elem.getAttribute()` is Element's. ★★ WHAT REFUSES IS THE POINT — `get` has 14 callable owners, `delete` 13, `has` 11, `add` 10, `set` 9, `addEventListener` 230, so the rung declines exactly the names a project is most likely to use for its own API. Resolutions carry `inferred` (the TYPE-INFERRED tier): the member is certain, the RECEIVER is a hedge. Purely additive, 0 removed; 48 sampled sites across jquery and converse.js were read and all 48 correct. Prior: ★ resolution 38.8 -> 49.5%. recalib 2026-09-06 (v188 THE BROWSER SURFACE, CART-0805): +21 refs, +8 nodes. `tools/domdistill.lua` reads TypeScript's own lib.dom.d.ts into dom-api.mpack — 27 globals, 18873 keys, extends chains expanded so `document.appendChild` (declared seven interfaces up, on Node) is a key — and spec/profile/node.lua unions it, because select_env gives a root ONE profile and a repo is routinely both. ★★ AND THE GATE THAT HAD TO MOVE FIRST: the stdlib TAIL word list refused every one of these calls as `refused: vocab` before the surface holding the exact dotted key was asked. An oracle that confirms the whole path outranks a hand-written tail word. Purely additive, 0 removed. Prior: ★ the 8 minted names are document.createElement/querySelector/querySelectorAll/addEventListener/removeEventListener, window.add/removeEventListener and JSON.parse — the last one is the NODE half of the same fix, tail-refused for just as long. recalib 2026-09-06 (v185 A SHAPE'S TWO CONFIG SOURCES COMPOSE, CART-0802): +17 refs, +7 nodes. `shapes.probe` joined a shape's static `config` and its `configure(root)` with an `or`, so node-package's `profile = 'node'` was thrown away on every package.json naming a `main` — and this corpus's does. The v182 node profile had therefore only ever activated on ghost, whose manifest names no entry. ★ THE TELL WAS A NUMBER THAT DID NOT MOVE: jquery and converse.js reported their pre-profile resolution to the decimal and I had a plausible wrong story ready for it. Purely additive, 0 removed; every minted name (parseFloat, isNaN, Date.now, String.fromCharCode, encodeURIComponent, eval) verified callable against `node` itself, and each is ECMAScript or a timer, so the claim holds for browser code too. Prior: -- recalib 2026-07-26: v104 js member-target function literals (+95 refs/+41 nodes defs): `X.y = function(){}` now mints, so pre-class exports are definitions. MEASURED per-item vs a HEAD baseline: recovered 36, +11 out of a refusal, 20 redirected (10 CORRECT — `find.error`/`find.matchesSelector` calls that had been landing on core.js's bare `error` and selector-native's `matchesSelector`; 2 WRONG — `jQuery.error(…)` now hits `find.error`, a tail collision the `tail or exact` bug at treesitter.lua:5381 keeps resolve_module_alias from correcting), 0 LOST. Prior: refs 1029 / nodes 811 -- +anon callback fns @ v51 (df-strangler B; +257 #cb nodes)
        lang = 'javascript',
    ...
    mootools = {
        ...
        rev = '187a16bae2d7',
        expected = { refs = 629, nodes = 598 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs -13, nodes +0. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: recalib 2026-09-07 (v189 THE UNIQUE-OWNER RUNG, CART-0806): +81 refs, +14 nodes. A member name that belongs to EXACTLY ONE callable type in the distilled surface, and that this project NEVER NAMES, types its own receiver: `elem.getAttribute()` is Element's. ★★ WHAT REFUSES IS THE POINT — `get` has 14 callable owners, `delete` 13, `has` 11, `add` 10, `set` 9, `addEventListener` 230, so the rung declines exactly the names a project is most likely to use for its own API. Resolutions carry `inferred` (the TYPE-INFERRED tier): the member is certain, the RECEIVER is a hedge. Purely additive, 0 removed; 48 sampled sites across jquery and converse.js were read and all 48 correct. Prior: ★ resolution 33.5 -> 42.6%. recalib 2026-09-06 (v188 THE BROWSER SURFACE, CART-0805): +29 refs, +8 nodes. `tools/domdistill.lua` reads TypeScript's own lib.dom.d.ts into dom-api.mpack — 27 globals, 18873 keys, extends chains expanded so `document.appendChild` (declared seven interfaces up, on Node) is a key — and spec/profile/node.lua unions it, because select_env gives a root ONE profile and a repo is routinely both. ★★ AND THE GATE THAT HAD TO MOVE FIRST: the stdlib TAIL word list refused every one of these calls as `refused: vocab` before the surface holding the exact dotted key was asked. An oracle that confirms the whole path outranks a hand-written tail word. Purely additive, 0 removed. Prior: recalib 2026-09-06 (v185 A SHAPE'S TWO CONFIG SOURCES COMPOSE, CART-0802): +37 refs, +13 nodes. `shapes.probe` joined a shape's static `config` and its `configure(root)` with an `or`, so node-package's `profile = 'node'` was thrown away on every package.json naming a `main` — and this corpus's does. The v182 node profile had therefore only ever activated on ghost, whose manifest names no entry. ★ THE TELL WAS A NUMBER THAT DID NOT MOVE: jquery and converse.js reported their pre-profile resolution to the decimal and I had a plausible wrong story ready for it. Purely additive, 0 removed; every minted name (parseFloat/parseInt, isNaN, set/clearTimeout, set/clearInterval, Date.now, String.fromCharCode, encodeURIComponent) verified callable against `node` itself, and each is ECMAScript or a timer, so the claim holds for browser code too. Prior: -- recalib 2026-07-31 (v107 MODULE-LEVEL OWNER): +52 refs (nodes unchanged) = top-level calls now owned by their statement-run REGION. Δrefs == edges added, 0 removed. Biggest relative lift of the js corpora (+12%) and it fits the culture: in prototype-extension code the module BODY is the program (Core.js::region@22 calling Function.overloadSetter). Prior: 443 -- recalib 2026-07-26: v104 js member-target function literals (+44 refs/+18 nodes defs): `X.y = function(){}` now mints, so pre-class exports are definitions. MEASURED per-item vs a HEAD baseline: recovered 4, +16 out of a refusal, 35 redirected — all `ua.match(/…/)`, i.e. String.prototype.match, where the old and new targets are both wrong (~-tier noise, not a regression), 0 LOST. Prior: refs 399 / nodes 545 -- +9/+3 @ v63 B4 prototype methods (Function.prototype.overloadSetter/String.prototype.contains)
        lang = 'javascript',
    ...
    synjava = {
        ...
        lang = 'java',
        expected = { refs = 55, nodes = 113 }, -- recalib 2026-08-24 (v154 A JAVA FIELD WITHOUT AN INITIALIZER IS STILL A FIELD, CART-0537): +14 nodes, Δrefs 0. The synthetic java fixture declares fields without initializers, which the `vars` query used to require a value for. Δrefs 0 is the tell that these are new NODES rather than re-pointed edges. ★ AND THIS CORPUS CAUGHT WHAT I UNDER-GATED: v154 was verified on libs and server and NOT on the synthetic java corpus, so the pin went stale for two versions until a roster --save reported it — the CART-0528 point (a synthetic corpus is the fence for a language's declarations) arriving from the opposite direction. Prior: 99, -- recalib @ gen v6 (CART-0377): +12 nodes / +8 refs = Ctrl.java, the CONTROL BESTIARY. synjava covered 2 of java's 13 control forms (only if/for), so the java gate could never have failed on java switches opening NOTHING — now 13/13. The eight numbered modules are byte-identical to v5, verified by diff. Prior: refs 47, nodes 87 -- calibrated @ gen v3, byte-identical @ v4
    ...
    nio = {
        ...
        rev = '21f5324bfac1',
        lang = 'lua',
        expected = { refs = 428, nodes = 537 }, -- recalib 2026-09-15 (v193 A NESTED RETURNED CLOSURE IS A NODE, CART-0926): refs +1, nodes +14. `(return_statement (expression_list (function_definition) @rdef))`, minted as `<enclosing>#ret` and ONLY when it has a function ancestor: a TOP-LEVEL `return function () … end` stays part of its REGION (user's decision 2026-09-15, and tests/toplevel_spec.lua:70 from a user report), while a NESTED one had NO OWNER AT ALL — the flow walk stops at `function_definition` and deletes the interior rows rather than relocating them. ★ NODES +14 AND REFS ONLY +1: the closures are mostly leaf helpers, so minting them moves almost no edge — the value is that their bodies are now ANALYSED, not that the graph grew. Split measured: 57 nested / 25 top-level here, 57 / 0 on wow. Prior: recalib 2026-09-15 (v192 A BRACKETED KEY IS STILL A KEY, CART-0927): refs +14, nodes +8. `(field name: (string|number) @name value: (function_definition) @def)` — the clause above asked for `name: (identifier)`, which `{ k = fn }` and `{ [K] = fn }` both satisfy, so a bracketed STRING or NUMBER key minted NOTHING. The eight here are gendocs.lua's `['@generic'] = function …` annotation handlers. ★★ AND THE REFS MOVE BECAUSE `fn_at` PICKS THE INNERMOST CONTAINING MINTED FUNCTION: edges from inside those bodies were attributed to the enclosing `create_config@679` and now belong to the handler itself — the same attribution move v191 made for inline callbacks, one position over. ⚠ A SINGLE-QUOTED KEY IS WHY THIS WAS NEARLY MISSED: a census grep for `["..."] = function` reported ZERO for this corpus and the gate still moved. Scale: wow 3031 unminted -> 120 (2911 discharged, 6.7% -> 0.3% of 45445 functions). Prior: recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs +137, nodes +139. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: calibrated 2026-08-03 @ 21f5324
