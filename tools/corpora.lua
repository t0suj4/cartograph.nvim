...
return {
    ...
    nio = {
        ...
        rev = '21f5324bfac1',
        lang = 'lua',
        expected = { refs = 413, nodes = 515 }, -- recalib 2026-09-07 (v191 A LUA INLINE CLOSURE IS A NODE + THE CALL-OWNER TEST IS POSITION-AWARE, CART-0813): refs +137, nodes +139. Two changes and the second was found by the first: `(arguments (function_definition)) @adef` mints `verb#cb` nodes the way the JS front end has since v51, and `fn_at` — which picks the innermost function CONTAINING a call — was LINE-GRANULAR, so a callback opening on its caller's line contained that call and the registration was attributed to its own argument. ★★ THE OWNER FIX MOVES ATTRIBUTION IN EVERY LANGUAGE WITH A SAME-LINE CALLBACK, not just lua: jquery gains `core.js::even -> core.js::grep` and loses the copy that hung off the callback, and the net is negative where the real owner already had that edge (addref dedupes by from/to). Prior: calibrated 2026-08-03 @ 21f5324
