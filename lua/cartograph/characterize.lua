...
local function satisfied_by(h)
    if h.kind == 'dependency' and h.rule == 'stdlib' and not h.hedged then
        return 'the runtime: a Lua interpreter holds this'
    end
...
function M.fill(plan, fills)
    ...
    for id, f in pairs(fills) do
        ...
        h.value, h.basis, h.by = tostring(f.value), f.basis, by
...
function M.emit(plan)
    ...
    add(('-- graph generation %s'):format(tostring(plan.generation)))
    add ''
    if #(plan.premises or {}) > 0 then
        add '-- PREMISES THIS SPEC ASSUMES (a stub is a supplied premise, not a fact):'
        for _, p in ipairs(plan.premises) do add('--   · ' .. p) end
        add ''
    end
    local supplied = {}
    for _, h in ipairs(plan.holes) do
        if h.basis then
            supplied[#supplied + 1] = ('%s = %s  [%s, by %s] %s'):format(h.id,
                h.value, h.filled_tier or '?', h.by or '?', h.basis)
        end
    end
    if #supplied > 0 then
    ...
    add ''
    add(('local got = %s(%s)'):format(plan.subject.expr or 'SUBJECT',
        table.concat(args, ', ')))
    -- THE ORACLE
    local oracle
    for _, h in ipairs(plan.holes) do
    ...
    if oracle then
        if oracle.value then
            add(('local want = %s  -- %s, by %s: %s'):format(oracle.value,
                oracle.filled_tier or '?', oracle.by or '?', oracle.basis or ''))
        else
            add(('local want = %s'):format(hole_call(oracle)))
        end
        add 'if got ~= want then'
        add ("    error(('CHANGED: %s returned %s, characterized as %s'):format("
            .. ('%q, tostring(got), tostring(want)))'):format(plan.fn))
        add 'end'
    else
...
function M.report(plan)
    ...
    for _, h in ipairs(plan.holes) do
        ...
        add(('  %s %-22s %-9s %s'):format(mark, h.id, h.tier or 'FRONTIER',
