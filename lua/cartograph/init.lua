    ...
    elseif vim.fn.isdirectory(target) == 1 then
        ...
        local function finish(data)
          ...
          local function enrich()
            ...
            -- the enrichment passes, declared once in cartograph.postpass (CART-0847): cross-language links,
            -- embedded SQL, the framework loops, the deployment/contract layers, the erlang tuple registry, a
            -- configured database. The loss report runs the same sequence, so both see the same final graph.
            require('cartograph.postpass').run(data, { say = function (msg, level)
                vim.notify('cartograph: ' .. msg, level)
            end })
