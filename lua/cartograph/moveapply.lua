...
local function collect(store, ids, dest, plan)
    ...
    for _, id in ipairs(ids) do
        ...
        elseif n.kind ~= 'function' and n.kind ~= 'method' then
            ...
            -- the predicate now lives in `txn` — CART-0773 needed the same
            -- rule on the DELETE side, and a second copy of a three-day-old
            -- soundness rule is the thing this project tells itself not to do
            local encl = txn.enclosing_syntax(n.file, n.range, file_text(n.file))
            if encl then
